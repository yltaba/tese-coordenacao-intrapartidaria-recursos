"""Gera narrativa visual autossuficiente a partir dos resultados auditados do cap. 3."""
...
section(2,'O núcleo financiado cresce — e a dispersão entre listas também','O NECr médio sobe de 2,96 para 5,90. O desvio-padrão passa de 3,17 para 4,87. As distribuições mostram por que a mediana, isoladamente, não descreve todos os resultados.')
...
section(5,'O Top-NECr contém a maioria dos eleitos','A cobertura nacional aumenta de 86,74% para 92,63%, enquanto a cobertura esperada por sorteio permanece próxima de 43,8%. Cobertura e precisão respondem a perguntas diferentes.')
fig=make_subplots(rows=1,cols=2,subplot_titles=['Quantos eleitos estavam no núcleo?','Quantos priorizados foram eleitos?'])
main=coverage[coverage['Regra de k']=='arredondado'].set_index('Ano')
for year in [2018,2022]:
    s=main.loc[year]
    ...
    fig.add_trace(go.Bar(x=['Precisão'],y=[s['Precisão nacional (%)']],name=str(year),legendgroup=str(year),showlegend=False,marker_color=COLORS[year],text=[number(s['Precisão nacional (%)'])+'%'],textposition='outside',hovertemplate='%{y:.2f}%<extra>%{fullData.name}</extra>'),row=1,col=2)
...
metrics={'NECr':'NECr','C':'Candidatos formais (C)','F':'Candidatos competitivos (F)','D':'C − NECr','Q':'C / NECr','NECr_Mp':'NECr / Mp','NECr_E':'NECr / eleitos','NECr_menos_E':'NECr − eleitos','Cobertura_pct':'Cobertura por nominata (%)','Precisao_pct':'Precisão por nominata (%)'}
...
nav=''.join(f'<a href="#s{i}">{label}</a>' for i,label in enumerate(['Competitivos','Distribuições','Bancada','Eleitos','Top-NECr','UFs','Explorar'],1))
doc='<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Capítulo 3 — Atlas visual dos resultados</title><style>'+css+'</style><script>'+get_plotlyjs()+'</script></head><body><header><div class="eyebrow" style="color:#a6cbd5">TESE / CAPÍTULO 3 / 2018 E 2022</div><h1>Como os recursos se distribuem dentro das nominatas?</h1><p>Uma leitura visual da competitividade, da concentração do financiamento e da correspondência com a bancada e os eleitos.</p><div class="legend"><span><i class="dot" style="background:#54b4d6"></i>2018</span><span><i class="dot" style="background:#f19b68"></i>2022</span></div><nav>'+nav+'</nav></header><main>'+''.join(chunks)+'</main><script>'+js+'</script></body></html>'
target=OUT/'atlas-visual-capitulo-3.html'
target.write_text(doc,encoding='utf-8')
assert len(figures)==12 and len(d)==1570
print(str(target))
