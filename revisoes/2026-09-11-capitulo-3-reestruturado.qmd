---
title: "Revisao — 03-medindo-coordenacao-intrapartidaria-reestruturado"
...
<!-- Revisão de 11/09/2026. Incorpora resultados-financiamento-alternativo/, resultados-teto-financiamento/ e exploracao-gastos-teto/, além dos resultados-capitulo-3/. O diálogo com a literatura foi conferido nos PDFs de textos_prioritarios; rastreabilidade em revisoes/fontes-discussao-capitulo3/mapa-evidencias.md. Os modelos fracionários legados são preservados sem reestimação; suas pendências de documentação permanecem explicitadas. O apêndice integra este arquivo. Top-Mp não foi calculado. -->
...
O número de candidaturas da nominata, $C_{puy}$, define seu tamanho formal, para o partido $p$, na UF $u$ e eleição $y$. Para descrever a distribuição financeira, utiliza-se o número efetivo de candidaturas em recursos, adaptando a medida de @laaksotaagepera1979. Essa aplicação possui antecedentes tanto na análise das receitas partidárias brasileiras de @bolognesietal2020 quanto na comparação entre dinheiro e votos de @thomsen2023:
...
Esses resultados indicam uma ampliação do conjunto de candidatos com credenciais anteriores em relação à representação partidária disponível. A distinção entre crescimento da lista e crescimento de seu segmento competitivo é relevante diante de @hottmenezes2023. Nas eleições municipais brasileiras de 2012 e 2016, os autores encontram que vagas adicionais nas nominatas são ocupadas por candidatos mais fracos, principalmente novatos, e que essa experiência pode favorecer sua eleição em uma tentativa posterior. Aqui, a participação dos candidatos com credenciais prévias cresce entre dois pleitos para deputado federal. A diferença não constitui refutação daquele resultado: trata-se de uma mudança agregada entre eleições, não do efeito causal de permitir uma candidatura adicional. Nos dois desenhos, o número formal de concorrentes, isoladamente, é insuficiente para caracterizar a composição da competição. Resta verificar quais desses candidatos são financiados.
...
O resultado dialoga com @silvacodato2024, que identificam redução da desigualdade financeira em 2022 em comparação com 2018, sem desaparecimento das grandes assimetrias entre candidatos. A convergência está na expansão do acesso aos recursos, mas as medidas não são intercambiáveis: aqueles autores examinam Gini, concentração no topo e diferenças entre faixas de arrecadação, enquanto as razões aqui apresentadas descrevem a distribuição intranominata e atribuem o mesmo peso a cada lista financiada. O NECr maior, portanto, não deve ser apresentado como replicação exata de uma redução do Gini. A pergunta adicional deste capítulo é se a ampliação do núcleo preserva sua focalização eleitoral.
...
Essa associação dá continuidade ao padrão de vantagens financeiras de candidatos com capital político sintetizado por @mancuso_2015_investimento. Para o período recente, @silvacodato2024 identificam uma vantagem especialmente elevada dos candidatos à reeleição em 2018 e seu recuo relativo em 2022, ainda em um contexto desigual. A aproximação entre os achados deve preservar a diferença de conceitos: uma vitória anterior em qualquer dos cargos considerados nesta tese não equivale a estar concorrendo à reeleição para deputado federal, e a participação individual na nominata não é a parcela nacional de recursos controlada pelos incumbentes. O resultado novo não é a existência de uma vantagem da experiência, mas sua articulação com a composição do núcleo financeiro e com a cobertura de candidaturas previamente competitivas.
...
Há também uma diferença em relação ao gradiente distrital descrito por @cheibubsin2020: a contenção da competição identificada por eles é mais acentuada nos distritos pequenos. Em 2018, a cobertura financeira aqui observada também diminui com a magnitude; em 2022, esse ordenamento desaparece. Isso não demonstra que a magnitude deixou de importar para a competição intrapartidária. A comparação entre candidatos fortes e cadeiras e a inclusão de vencedores em um ranking de receitas captam dimensões diferentes. O achado delimita o alcance da comparação: em 2022, as grandes magnitudes não impedem uma correspondência elevada entre o núcleo financeiro e os eleitos.
...
Essa distinção qualifica a aproximação com @thomsen2023. Nas primárias para a Câmara dos Representantes dos Estados Unidos de 1980 a 2020, a autora mostra que a competição parece menor quando medida pela arrecadação do que pelos votos, embora uma minoria de candidaturas financeiramente desfavorecidas vença. Um eleito fora do Top-NECr, porém, não é necessariamente um candidato pouco financiado no total: está fora de um ranking de uma origem específica. A maior arrecadação alternativa desses vencedores mostra por que a dimensão partidária não deve ser confundida com a disponibilidade financeira completa da campanha. Tampouco se replica aqui a comparação entre número efetivo em votos e em receitas realizada por Thomsen.
...
O diagnóstico do teto converge com uma observação já feita por @silvacodato2024 [p. 14]: o pequeno número de candidaturas com arrecadação próxima ao limite não seria suficiente para explicar a distribuição mais ampla de recursos em 2022. A exploração desta tese acrescenta o recorte por resultado e posição no Top-NECr, utilizando uma aproximação de despesas. Entre candidatos com despesas registradas, apenas 0,41% em 2018 e 0,46% em 2022 alcançam pelo menos 95% do limite. Entre os eleitos, são 2,73% e 6,24%; entre eleitos fora do núcleo, os gastos medianos equivalem a 9,49% e 15,93% do teto. As porcentagens não são diretamente comparáveis às daquele estudo, que examina arrecadação e outro corte de proximidade. A distância observada não refuta complementação até um orçamento desejado inferior ao máximo legal, mas não oferece base para tratar o teto como restrição ativa generalizada. Seu papel permanece descritivo e de escala.[^03-teto-diagnostico]
...
Os resultados permitem retomar a mudança de perspectiva discutida no capítulo anterior: incentivos ao voto pessoal não eliminam a capacidade dos partidos de estruturar a competição. A contribuição empírica está em mostrar como uma dimensão dessa estruturação aparece nas receitas partidárias. Entre 2018 e 2022, o número efetivo financiado se amplia, enquanto a presença de competitivos prévios e de eleitos no núcleo permanece muito superior à esperada por sorteio. A interpretação desse padrão exige situá-lo diante de três conjuntos de resultados da literatura: a composição seletiva das listas, a distribuição desigual dos recursos e a autonomia financeira das candidaturas.

### Da composição das listas à identificação de um núcleo financeiro

@cheibubsin2020 [pp. 83–85] mostram que, entre as listas com candidatos fortes examinadas de 2002 a 2014, aproximadamente 65% apresentavam no máximo um candidato forte a mais que o número de eleitos. Essa evidência desloca o foco do número bruto de candidatos para o segmento efetivamente competitivo da nominata. Os resultados deste capítulo convergem com a ideia de diferenciação interna, mas modificam seu objeto: o NECr delimita a amplitude do financiamento partidário e o Top-NECr identifica quem ocupa seu núcleo. As proporções de listas com até um competitivo a mais que $Mp$, de 84,93% em 2018 e 45,63% em 2022, não formam uma série diretamente comparável aos 65% daquele estudo, pois mudam a referência de cadeiras, a definição de competitividade e o universo condicionado. O afastamento em relação à bancada anterior tampouco impede que o núcleo financeiro conserve forte correspondência eleitoral.

A passagem do tamanho do núcleo à identidade de seus integrantes também aprofunda uma questão já presente em @bolognesietal2020. Para 2014, os autores encontram forte correlação entre o número efetivo de financiados pelos partidos e as bancadas conquistadas ($r=0{,}924$), associando concentração financeira e desempenho eleitoral [@bolognesietal2020, pp. 54–56]. Uma correlação entre quantidades, entretanto, não estabelece por si só se os financiados e os eleitos são as mesmas pessoas. A cobertura de 86,74% e 92,63%, acompanhada dos lifts de 1,98 e 2,12, acrescenta essa verificação nominal e um parâmetro que preserva o tamanho de cada núcleo. O avanço é demonstrar a correspondência para além da escala do partido, sem fazer da coincidência entre número efetivo e cadeiras uma condição necessária de coordenação.

Esse resultado complementa o argumento de *gatekeeping* posterior à indicação apresentado por @janusz_barreiro_cintron_2021: permitir a candidatura não encerra a intervenção partidária sobre as condições da campanha. Contudo, o controle de recursos não deve ser confundido com seleção neutra por mérito eleitoral. Os autores encontram desvantagens para mulheres na distribuição de dinheiro, tempo de televisão e números de candidatura, inclusive após considerar diferenças de experiência e qualidade dos candidatos. A elevada cobertura dos eleitos no Top-NECr, portanto, não demonstra igualdade de oportunidades nem legitima os critérios distributivos: pode coexistir com barreiras que contribuem para definir quem acumula credenciais e consegue vencer. O indicador mede focalização financeira, não inclusão ou democracia interna.

### Expansão do financiamento sem desaparecimento da seletividade

A ampliação do NECr e a redução mais moderada de algumas medidas de concentração devem ser lidas à luz de @silvacodato2024. Os autores mostram que 2018 e 2022, embora marcados pela predominância de recursos públicos, não apresentam a mesma distribuição: a segunda eleição é menos desigual, mas preserva vantagens relevantes para políticos profissionais e incumbentes. A evidência deste capítulo acrescenta que a distribuição mais ampla pode coexistir com focalização persistente: a cobertura de competitivos prévios passa de 80,90% para 82,68%, e a dos eleitos aumenta sem perder sua vantagem sobre o sorteio. Assim, reduzir a concentração não equivale necessariamente a enfraquecer a seleção financeira, da mesma forma que concentrar mais não demonstra, por si só, maior coordenação.

A queda da precisão, de 19,20% para 12,43%, explicita o outro lado dessa expansão: financiar um núcleo maior incorpora mais vencedores, mas também mais candidaturas derrotadas. O resultado dialoga com o dilema entre favorecer candidatos específicos e incentivar o esforço coletivo examinado por @fivaetal2024 nas eleições locais norueguesas. Nesse estudo, partidos eleitoralmente mais fortes conseguem proteger preferidos com menor comprometimento do desempenho coletivo. Não se observa aqui o esforço dos candidatos ou o mesmo mecanismo de posições de vantagem, de modo que a expansão brasileira não constitui um teste daquele modelo. A comparação é útil para mostrar por que máxima concentração, máxima precisão e melhor desempenho partidário não são objetivos necessariamente equivalentes.

As associações com o histórico eleitoral, por sua vez, prolongam a literatura sobre vantagens financeiras de incumbentes e candidatos com capital político sintetizada por @mancuso_2015_investimento. Elas não resolvem a direção do poder no interior dos partidos. @bolognesietal2020 interpretam a concentração em poucos nomes também como expressão da primazia de candidatos sobre a organização. Essa leitura é uma alternativa à interpretação de comando das lideranças: candidaturas fortes podem receber recursos porque são priorizadas, porque têm poder para obtê-los ou porque ambos os processos se combinam. A focalização observada é compatível com coordenação por meio dos recursos, mas não discrimina essas relações organizacionais.

Também não se atribui a ampliação de 2022 a um efeito identificado do FEFC, das cotas ou das mudanças nas coligações. @silvacodato2024 discutem hipóteses institucionais e de adaptação partidária para a mudança distributiva, sem isolar a contribuição de cada uma. Os resultados aqui acrescentados são consistentes com uma reorganização do financiamento, mas a comparação entre dois pleitos e a manutenção da unidade partido × UF não resolvem esse problema. A mesma cautela vale para o teto: a exploração confirma e detalha a insuficiência de uma explicação baseada em saturação generalizada do limite, já apontada naquele trabalho.

### Recursos alternativos e o alcance da inferência sobre estratégia

Os eleitos fora do núcleo mostram que diferenciação partidária e autonomia individual podem coexistir. Esse resultado se aproxima da perspectiva de @samuels1999, segundo a qual as estratégias dos candidatos dependem, entre outros fatores, do acesso e do controle sobre recursos, e não apenas dos incentivos comuns do sistema eleitoral. A contribuição específica desta tese é caracterizar os vencedores que escapam ao núcleo partidário: eles dispõem de muito mais receitas alternativas que os não eleitos também situados fora. O achado delimita uma dimensão da competição que o ranking partidário, por definição, não incorpora.

Essa heterogeneidade não contradiz os resultados sobre gastos e competição discutidos no capítulo anterior. @samuels_2001_every_penny [pp. 96–98] não encontra associação entre o número bruto de concorrentes e os gastos, mas identifica uma associação positiva com a qualidade dos correligionários; a evidência varia conforme a operacionalização da competição. O estudo pergunta como as características dos concorrentes se relacionam com os gastos de uma candidatura. Aqui, a pergunta é como os recursos de origem partidária se distribuem e se relacionam com outras fontes. A associação positiva entre recursos privados e partidários no universo completo não testa uma corrida de gastos entre correligionários e não permite refutar ou confirmar aquele mecanismo. Ela apenas contraria a expectativa de substituição média sistemática nas especificações estimadas.

O diálogo com @thomsen2023 também exige preservar o objeto e a temporalidade da medida. Seu achado de maior concentração nas receitas do que nos votos sustenta a utilidade de observar a competição por outra dimensão financeira, sem equiparar as duas distribuições. A autora utiliza receitas anteriores às primárias, enquanto este capítulo trabalha predominantemente com receitas acumuladas nas contas finais e separa janelas apenas no exercício temporal exploratório. Além disso, ela examina a arrecadação da campanha, e o NECr aqui se restringe à origem partidária. Por isso, um vencedor fora do Top-NECr não equivale automaticamente ao vencedor financeiramente desfavorecido em seu estudo. A distinção reforça a decisão de investigar outras fontes sem adicioná-las ao indicador principal.

O conjunto dos resultados sustenta, assim, uma extensão delimitada da literatura: a seletividade financeira possui conteúdo eleitoral, mesmo quando o núcleo se amplia, e seus casos de não cobertura apresentam um perfil de arrecadação alternativa distinto. Não demonstra que os partidos conscientemente compensam essa capacidade, pois os modelos entre competitivos prévios e o exercício temporal permanecem inconclusivos. Tampouco permite denominar a cobertura uma estimativa conservadora da estratégia. A vitória combina financiamento, atributos dos candidatos e resposta dos eleitores, sem revelar retrospectivamente a intenção das lideranças. O capítulo seguinte retoma o momento dos repasses para investigar a antecipação financeira, preservando a distinção entre observar a distribuição final e identificar o processo pelo qual ela se constitui.
